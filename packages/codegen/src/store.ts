...
export function renderStore(_spec: ProjectSpec): string {
  return [
    ...
    '      else {',
    '        const maxId = rows.reduce((max, existing) => {',
    '          const m = /^row-(\\d+)$/.exec(existing.id)',
    '          return m ? Math.max(max, Number(m[1]!)) : max',
    '        }, 0)',
    '        rows.push({ ...row, id: `row-${maxId + 1}` } as Row)',
    '      }',
    '    },',
    '    remove(id: string): void {',
    ...
    "    lines.push(headers.map((header) => escapeCell(String(row[header] ?? ''))).join(','))",
    '  }',
    "  return lines.join('\\n')",
    '}',
    '',
